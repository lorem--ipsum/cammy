...
.factory('$herlock', function($colors) {
  return {
    dots: function(n) {
      var step = 1/(n-1);
      var vertices = [];
    ...
    link: function(n, vertices) {
      var a = [];
      
      for (var x = 0; x < n; x++) {
        for (var y = x*n; y < x*n+n - 1; y++) {
          a.push({from: y, to: y+1});
        }
      }
      
      for (var y = 0; y < n; y++) {
        for (var x = 0; x < n-1; x++) {
          a.push({from: y + n*x, to: y + n*(x+1)})
        }
      }
    ...
    applyColorToLines: function(lines, vertices) {
      lines.map(function(line) {
        line.color = vertices[line.from].color;
      });
    }
  };
})
